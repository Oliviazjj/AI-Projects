...
public class Search {
	...
	public String IterativeDeepeningTreeSearch() {	
		for(int i=0; ; i++){
			String result=LimitedTreeSearch(new FrontierLIFO(), i);
	...
	public String IterativeDeepeningGraphSearch() {
		for(int i=0; ; i++){
			String result = LimitedGraphSearch(new FrontierLIFO(), i);
			if(result != null) return result;
		}	
	...
	private String LimitedTreeSearch(Frontier frontier, int limit) {
		cnt = 0; 
		node_list = new ArrayList<Node>();
	...
	private String LimitedGraphSearch(Frontier frontier, int limit) {
		cnt = 0; 
		node_list = new ArrayList<Node>();
	...
	public String PrintTree(){
		return ""+PrintTree(node_list.get(0));
	}
	private String PrintTree(Node n) {
		...
		if(n.order < 0){
			process += String.format("%s(g=%.2f, h=%.2f, f=%.2f)\n", 
			n.state, n.path_cost, problem.h(n.state), n.path_cost+problem.h(n.state));
		}else{
			process += String.format("%s(g=%.2f, h=%.2f, f=%.2f) order=%d\n", 
			n.state, n.path_cost, problem.h(n.state), n.path_cost+problem.h(n.state), n.order);
		}
		
		for (Node m: node_list){
			if(m.parent_node == n) process+=PrintTree(m);
